...
apply_banner() {
    ...
    echo -e "${G}Banner activado correctamente.${N}"
}
...
show_banner_menu() {
    ...
    echo -e "${R}══════════════════════════ / / / ══════════════════════════${N}"
    echo -e "${R}[${Y}1${R}]${N}  ${C}CREAR / EDITAR BANNER (SN-Manager)${N}"
...
create_edit_banner() {
    ...
    # Si el archivo no existe o está vacío, ponemos el banner predeterminado
    if [[ ! -s "$BANNER_FILE" ]]; then
        echo -e '<font color="#00FF00">SN</font><font color="#FFFFFF">-</font><font color="#FF0000">Manager</font>' > "$BANNER_FILE"
        echo -e "${G}Se ha generado el banner predeterminado.${N}"
    fi

    echo -e "${Y}Se abrirá nano. Puedes añadir más texto si lo deseas.${N}"
